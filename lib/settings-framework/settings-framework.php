...
if(!class_exists('vex_press_settings')) :
	/**
	 * vex_press_settings class
	 */
	class vex_press_settings {
		...
		public function __construct($settings_file, $option_group = '') {
			...
			if(!is_file($settings_file)) {
				exit(__('Settings file could not be found.', 'vex-press'));
			}
			...
			if($option_group) {
				...
				$this->option_group = $vex_press->get_option_group(basename($settings_file, '.php'));
			}

			$this->settings = array();
			$this->settings = apply_filters('vex_press_register_settings', $this->settings);
			if(!is_array($this->settings)) {
				exit(__('Settings framework must be an array', 'vex-press'));
			}
		...
		public function generate_setting($args) {
			...
			$this->setting_defaults = apply_filters('vex_press_defaults', $this->setting_defaults);
			...
			do_action('vex_press_before_field');
			do_action('vex_press_before_field_'.$field_id);
			switch($type) {
				case 'text':
				...
				default:
					...
					do_action('vex_press_custom_field');
					break;
			}
			do_action('vex_press_after_field');
			do_action('vex_press_after_field_'.$field_id);
		}
		...
		public function settings() {
			...
			if(isset($_POST['vex_press_uninstall'])) {
				check_admin_referer('vex-press-uninstall', 'vex-press-uninstall-nonce');
				delete_option($this->option_group);
				?>
				<div class="updated">
					<p><?php _e('All options have been removed from the database.', 'vex-press'); ?>

						<?php
						if(defined('VEXPRESS_PLUGIN_SLUG') && VEXPRESS_PLUGIN_SLUG != '') {
							$deactivate_url = 'plugins.php?action=deactivate&amp;plugin='.VEXPRESS_PLUGIN_SLUG.'/'.VEXPRESS_PLUGIN_SLUG.'.php';
							$deactivate_url = wp_nonce_url($deactivate_url, 'deactivate-plugin_'.VEXPRESS_PLUGIN_SLUG.'/'.VEXPRESS_PLUGIN_SLUG.'.php');
						} else {
							$deactivate_url = admin_url('plugins.php');
						}
						?>

						<?php printf(__('To complete the uninstall <a href="%1$s"">deactivate %2$s.</a>', 'vex-press'), esc_url($deactivate_url), VEXPRESS_PLUGIN_NAME); ?>
					</p>
				</div>
				<?php
				return;
			...
			if(isset($_POST['vex_press_reset'])) {
				check_admin_referer('vex-press-reset', 'vex-press-reset-nonce');
				delete_option($this->option_group);
				?>
				<div class="updated">
					<p><?php _e('All options have been restored to their default values.', 'vex-press'); ?></p>
				</div>
			<?php
			}

			do_action('vex_press_before_settings');
			?>
			<form action="options.php" method="post">
				<?php do_action('vex_press_before_settings_fields'); ?>
				<?php settings_fields($this->option_group); ?>
				<?php do_settings_sections($this->option_group); ?>
				<?php do_action('vex_press_after_settings_fields'); ?>

				<p class="submit">
					<input type="submit" class="button-primary" value="<?php _e('Save Changes', 'vex-press'); ?>" />

					<?php if($this->show_reset_button == TRUE) : ?>
						<input class="button-secondary" type="button" value="<?php _e('Restore Defaults', 'vex-press'); ?>" onclick="document.getElementById('vex-press-reset').style.display = 'block';document.getElementById('vex-press-uninst').style.display = 'none';" />
					<?php endif; ?>

					<?php if($this->show_uninstall_button == TRUE) : ?>
						<input class="button-secondary" type="button" value="<?php _e('Uninstall', 'vex-press'); ?>" onclick="document.getElementById('vex-press-uninst').style.display = 'block';document.getElementById('vex-press-reset').style.display = 'none';" />
					<?php endif; ?>
				</p>
			</form>

			<div id="vex-press-reset" style="display:none; clear: both;">
				<form method="post" action="">
					<?php wp_nonce_field('vex-press-reset', 'vex-press-reset-nonce'); ?>
					<label style="font-weight:normal;">
						<?php printf(__('Do you wish to <strong>completely reset</strong> the default options for', 'vex-press')); ?> <?php echo VEXPRESS_PLUGIN_NAME ?>? </label>
					<input class="button-secondary" type="button" name="cancel" value="<?php _e('Cancel', 'vex-press'); ?>" onclick="document.getElementById('vex-press-reset').style.display='none';" style="margin-left:20px" />
					<input class="button-primary" type="submit" name="vex_press_reset" value="Restore Defaults" />
				</form>
			</div>
			<div id="vex-press-uninst" style="display:none; clear: both;">
				<form method="post" action="">
					<?php wp_nonce_field('vex-press-uninstall', 'vex-press-uninstall-nonce'); ?>
					<label style="font-weight:normal;">
						<?php echo sprintf(__('Do you wish to <strong>completely uninstall</strong>', 'vex-press')); ?> <?php echo VEXPRESS_PLUGIN_NAME ?>?</label>
					<input class="button-secondary" type="button" name="cancel" value="<?php _e('Cancel', 'vex-press'); ?>" onclick="document.getElementById('vex-press-uninst').style.display = 'none';" style="margin-left:20px" />
					<input class="button-primary" type="submit" name="vex_press_uninstall" value="Uninstall" />
				</form>
			</div>
			<?php
			do_action('vex_press_after_settings');
		}
	}
endif;
